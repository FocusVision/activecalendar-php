...
class activeCalendar
{
	...
	*/
	public function activeCalendar($year=false,$month=false,$day=false,$GMTDiff="none")
	{
		...
		if($this->unixtime==-1 OR !$year) $this->unixtime=$this->timetoday;
		...
		if($this->isSchalt==1 AND $this->actmonth==2) $this->maxdays=29;
		elseif($this->isSchalt!=1 AND $this->actmonth==2) $this->maxdays=28;
	...
	*/
	public function enableWeekNav($link=false,$arrowBack=false,$arrowForw=false)
	{
		...
		if($arrowBack) $this->weekNavBack=$arrowBack;
		if($arrowForw) $this->weekNavForw=$arrowForw;
		$this->weekNav=true;
	}
	/*
	********************************************************************************
	...
	*/
	public function enableDatePicker($startYear=false,$endYear=false,$link=false,$button=false)
	{
		...
		{
			if($startYear>=$this->startYear AND $startYear<$this->endYear) $this->startYear=$startYear;
			if($endYear>$this->startYear AND $endYear<=$this->endYear) $this->endYear=$endYear;
		}
	...
	*/
	...
	{
		if(!is_bool($data)) throw new Exception('Incoming data was not as expected: Boolean needed!');
		$this->ajax=$data;
	}
	...
	{
		if(!is_scalar($data)) throw new Exception('Incoming data was not as expected: Integer or String needed!');
		$this->uniqueid=$data;
	}
	...
	{
		if(!is_array($data)) throw new Exception('Incoming data was not as expected: Array needed!');
	...
	{
		if(!is_string($data) AND $data!==false) throw new Exception('Incoming data was not as expected: String needed!');
	...
	public function setTimeFormat($data)
	{
		...
		if(!is_string($data)) throw new Exception('Incoming data was not as expected: String needed!');
		if(!in_array($data,$validTimeFormats)) throw new Exception('Incoming data was not as expected! Valid formats are: '.implode(', ',$validTimeFormats).'!');
	...
	public function setEventContent($year,$month,$day,$hour=false,$minute=false,$content,$url=false,$class=false,$id=false)
	{
		...
		if(!$class) $class=$this->cssEventContent;
	...
	*/
	...
	{
		if(!is_array($namesArray) OR count($namesArray)!=12) return false;
	...
	*/
	...
	{
		if(!is_array($namesArray) OR count($namesArray)!=7) return false;
	...
	*/
	...
	{
		if($this->rowCount<1 OR $this->rowCount>12) $this->rowCount=4;
		if(!$this->yearNav)
		{
	...
	*/
	...
	{
		if(!$stmonth OR $stmonth>12) $stmonth=1;
	...
	*/
	private function mkMonthTitle()
	{
		...
		elseif($this->ajax)
		{
			...
			if($this->weekViewToggleData!==false) echo '<td style="text-align:right; width:35px;">'.$this->weekViewToggleData.'</td>';
		...
		else
		{
			...
			if($this->actmonth==1) echo $this->mkUrl($this->actyear-1,'12');
			else echo $this->mkUrl($this->actyear,$this->actmonth-1);
			...
			echo '<div class="'.$this->cssMonthNav.'">';
			if($this->actmonth==12) echo $this->mkUrl($this->actyear+1,'1');
			else echo $this->mkUrl($this->actyear,$this->actmonth+1);
			echo $this->monthNavForw.'</a>';
			if($this->weekViewToggleData!==false) echo $this->weekViewToggleData;
			echo '</div></td></tr>'."\n";
		}
		return ob_get_clean();
	}
	...
	*/
	...
	private function mkMonthBody($showNoMonthDays=false)
	{
		...
		if($this->weekNum) echo '<td class="'.$this->cssWeekNum.'">'.$this->mkWeekNum(1+$cor).'</td>'."\n";
		$monthday=0;
		$nmonthday=1;
		for($x=0; $x<=6; $x++)
		{
		...
		for($x=0; $x<=6; $x++)
		{
			...
			if($stop==1) break;
			echo '<tr>';
			if($this->weekNum) echo '<td class="'.$this->cssWeekNum.'">'.$this->mkWeekNum($goon+$cor).'</td>'."\n";
	...
	{
		ob_start();
		if(!$month) $month=$this->actmonth;
		if(!$year) $year=$this->actyear;
		...
		if($this->javaScriptDay) $linkstr="<a href=\"javascript:".$this->javaScriptDay."(".$year.",".$month.",".$day.")\">".$day."</a>";
		...
		if(!$this->dayLinks) echo '<td class="'.$class.(($this->isEvent($day,$month,$year))? ' '.$this->eventID:'').'"><div class="dropbox" id="'.$id.'">'.(($this->eventUrl)?'<a href="'.$this->eventUrl.'">':'').$day.(($this->eventUrl)?'</a>':'').'<div>'.$eventContent.'</div></div></td>'."\n";
		else echo '<td class="'.$class.'"><div class="dropbox" id="'.$id.'">'.$linkstr.'<div>'.$eventContent.'</div></div></td>'."\n";
	...
	*/
	private function mkUrl($year,$month=false,$day=false)
	{
		...
		if($year AND $month AND $day) return $dayLink;
		if($year AND !$month AND !$day) return $yearNavLink;
		if($year AND $month AND !$day) return $monthNavLink;
	}
	...
	private function mkUrlAJAX($year,$month=false,$week=false,$day=false)
	{
		...
		$ajaxParams[]=(int)$year;
		if($year AND !$week AND !$month AND !$day) return '<span class="button" style="display:inline-block; width:30px; cursor:pointer;" onClick="'.$this->yearJSname.'(\''.implode('\',\'',$ajaxParams).'\');">';

		// generate links for week view
		if($week!==false)
		{
			$ajaxParams[]=(int)$week;
			if($year AND $week)  return '<span class="button" style="display:inline-block; width:30px; cursor:pointer;" onClick="'.$this->weekJSname.'(\''.implode('\',\'',$ajaxParams).'\');">';
		}

		// generate links for month view
		$ajaxParams[]=(int)$month;
		if($year AND $month AND !$day)  return '<span class="button" style="display:inline-block; width:30px; cursor:pointer;" onClick="'.$this->monthJSname.'(\''.implode('\',\'',$ajaxParams).'\');">';

		// generate links for day view
		$ajaxParams[]=(int)$day;
		if($year AND $month AND $day) return '<span class="button" style="display:inline-block; width:30px; cursor:pointer;" onClick="'.$this->dayJSname.'(\''.implode('\',\'',$ajaxParams).'\');">'.$day.'</span>';
	}
	...
	*/
	private function mkWeekNum($var)
	{
		...
		if($week>50 AND $this->actmonth==1) $year=$this->actyear-1;
		$out="";
	...
	*/
	...
	private function getWeekName($month=false)
	{
		// print the month
		if(!$month) $month=@$this->actmonth;
		if($this->monthNames) echo $this->monthNames[$month-1];
		else
		{
			switch($month)
			{
				case 1: echo $this->jan;  break;
				case 2: echo $this->feb;  break;
				case 3: echo $this->mar;  break;
				case 4: echo $this->apr;  break;
				case 5: echo $this->may;  break;
				case 6: echo $this->jun;  break;
				case 7: echo $this->jul;  break;
				case 8: echo $this->aug;  break;
				case 9: echo $this->sep;  break;
				case 10: echo $this->oct; break;
				case 11: echo $this->nov; break;
				case 12: echo $this->dec; break;
			}
		}
	...
	*/
	private function getMonthDays($month,$year)
	{
		...
		if($isSchalt==1 AND $month==2) $maxdays=29;
		elseif($isSchalt!=1 AND $month==2) $maxdays=28;
